...
class MatchSim {
    ...
    if(delivery==='low'){
      ...
      this._startTravel(o,target,'pass',()=>{
        ...
        if(!chance(win)){
          ...
          if(chance(.55)) this._setCorner(o.team); else this._turnover(def||gk);
          return;
        }
  ...
  _shoot(o, dtg, longshot, volley) {
    ...
    // COLOCAÇÃO DO GOLEIRO PESA (§realismo): bem postado, fecha o gol; batido
    // ou adiantado demais, entrega. Antes isto valia zero para o resultado.
    const gkCov=this._gkCoverage(gk,o.x,o.y,g);
    pGoal*=lerp(1.16,.78,gkCov);
    ...
    if(chance(pGoal)){
      ...
      const r2=R();
      // goleiro bem colocado também converte mais não-gols em DEFESA (e não em
      // bola pra fora): quem está na linha do chute alcança o que voa no gol.
      const saveCut=CAL.shooting.savedShare+gkQual*CAL.shooting.keeperSaveInfluence+(oneOnOne?.04:0)+gkCov*.07;
      /* BLOQUEIO EXIGE CORPO NA FRENTE (§realismo): a fatia de bloqueio era
         FIXA — encher a área de zagueiro não bloqueava um chute a mais, e o
         trabalho defensivo não aparecia em lugar nenhum. Agora conta quantos
         defensores estão de fato na linha do chute; é daí que saem o bloqueio
         e o escanteio que faltavam. */
      let laneDefs=0;
      for(const d of this.teams[1-o.team].players){
        if(d.red||d.isGK)continue;
        const tl=clamp(this._projT(o.x,o.y,g.x,g.y,d.x,d.y),0,1);
        if(tl<.08||tl>.92)continue;
        if(D(d.x,d.y,lerp(o.x,g.x,tl),lerp(o.y,g.y,tl))<1.9)laneDefs++;
      }
      const blockCut=saveCut+CAL.shooting.blockedShare*(oneOnOne?.55:1)*clamp(.55+laneDefs*.45,.55,1.95);
  ...
  _goalkeeperTarget(tm,p,b,dt) {
    ...
    /* FECHAR O ÂNGULO (§realismo). O goleiro acompanhava apenas uma fração do
       deslocamento lateral da bola (22-56%) e ficava quase sempre central —
       num chute de ângulo ele estava fora da linha bola→gol, com meio gol
       aberto, e defendia assim mesmo porque a POSIÇÃO não entrava na conta do
       chute. Agora ele se posta sobre a bissetriz (linha referência→centro do
       gol), que é como um goleiro de verdade fecha o ângulo. A precisão dessa
       colocação vem de POSICIONAMENTO: goleiro fraco fica central demais e é
       exatamente por aí que o gol no canto passa. */
    const place=(rx,ry,dep)=>{
      const ang=Math.atan2(ry-goal.y,rx-goal.x);
      const acc=clamp(.40+pos/145,.40,1);
      return [lerp(goal.x+tm.attackDir*dep,goal.x+Math.cos(ang)*dep,acc),
              clamp(lerp(goal.y,goal.y+Math.sin(ang)*dep,acc),goal.y-9,goal.y+9)];
    };
    let ty=place(b.x,b.y,depth)[1];
    ...
    if(attacker){
      ...
      if(danger<34){
        ...
        ty=place(attacker.x,attacker.y,depth)[1];
      }
  ...
  /* COBERTURA GEOMÉTRICA DO GOLEIRO (§realismo): 0 = totalmente batido (fora
     da linha de chute ou saiu na hora errada), 1 = perfeitamente colocado.
     Até aqui o desfecho do chute ignorava onde o goleiro estava — só o
     atributo pesava, então um goleiro pego no contrapé defendia igual a um
     bem postado. É isto que transforma boa colocação em defesa. */
  _gkCoverage(gk, sx, sy, g) {
    if (!gk || gk.red) return 0.34;
    const t = clamp(this._projT(sx, sy, g.x, g.y, gk.x, gk.y), 0, 1);
    const offLine = D(gk.x, gk.y, lerp(sx, g.x, t), lerp(sy, g.y, t));
    const dist = D(sx, sy, g.x, g.y);
    // sair fecha o ângulo, mas sair DEMAIS deixa a cavadinha e o drible
    const ideal = clamp(dist * 0.20, 1.4, 7);
    const depthErr = Math.abs(Math.abs(gk.x - g.x) - ideal) / Math.max(ideal, 1);
    return clamp(1 - offLine / 6.0, 0, 1) * clamp(1 - depthErr * 0.55, 0.22, 1);
  }
    ...
    for (const tm of this.teams) {
      ...
      for (const p of tm.players) {
        ...
        if (p === b.owner) {
          ...
          /* RECOMPOSIÇÃO (§realismo). Medido: com a bola a menos de 14 m do
             gol, o ALVO do zagueiro era 9,3 m do gol — correto, dentro da
             área — mas ele estava de fato a 19,3 m. DEZ metros de atraso. O
             alvo nunca foi o problema: o jogador voltava TROTANDO (a marcha
             que respira dava 55-80% de esforço) e ainda passava pelo filtro
             de reação, que num alvo em movimento vira erro permanente. Por
             isso a área ficava vazia. Defensor fora do posto com a bola em
             zona de perigo agora recompõe a fundo e sem atraso de leitura. */
          p._recovering = D(b.x, b.y, tm.goal.x, tm.goal.y) < 34 &&
                          D(p.x, p.y, tx, ty) > 3.5;
        }
        ...
        if (!ballDuty) {
          // BLOCO COMPRIME (§realismo): 5,5 m de separação entre companheiros é
          // espaçamento de jogo aberto. Defendendo a própria área, essa força
          // empurrava a zaga PARA FORA justamente quando ela precisa juntar —
          // seis jogadores não cabem na área com 5,5 m entre cada um. Sob
          // perigo, o raio encolhe e o bloco fecha.
          const defendingDeep = !attacking && D(b.x,b.y,tm.goal.x,tm.goal.y) < 30;
          const sepR = defendingDeep ? 3.1 : 5.5;
          let sepx = 0, sepy = 0;
          for (const q of tm.players) {                    // companheiros
            if (q === p || q.red) continue;
            const dd = D(p.x,p.y,q.x,q.y);
            if (dd < sepR && dd > 0.01) { sepx += (p.x-q.x)/dd * (sepR-dd); sepy += (p.y-q.y)/dd * (sepR-dd); }
          }
          const opp = this.teams[1 - tm.side];
          // Defendendo a própria área o zagueiro NÃO se afasta do atacante: a
          // repulsão o empurrava para longe justamente de quem ele deveria
          // marcar. Fora dessa zona ela continua valendo (a marcação respira).
          const oppW = defendingDeep ? 0.15 : 0.9;
          for (const q of opp.players) {                   // adversários: raio curto
            if (q.red) continue;
            const dd = D(p.x,p.y,q.x,q.y);
            if (dd < 3.2 && dd > 0.01) { sepx += (p.x-q.x)/dd * (3.2-dd) * oppW; sepy += (p.y-q.y)/dd * (3.2-dd) * oppW; }
          }
          /* TETO DA SEPARAÇÃO (§realismo) — a causa medida da área vazia. Esta
             força era um deslocamento LIVRE somado ao alvo tático. Com vários
             vizinhos perto (exatamente o que acontece defendendo a área), a
             soma superava o próprio alvo e expulsava o bloco: os zagueiros
             equilibravam num raio FORA da área, por mais que a tática mandasse
             entrar. Agora ela AJUSTA o alvo, nunca o substitui. */
          let dxs = sepx * 1.5, dys = sepy * 1.5;
          const mag = Math.hypot(dxs, dys), cap = defendingDeep ? 1.8 : 4.0;
          if (mag > cap) { dxs = dxs / mag * cap; dys = dys / mag * cap; }
          tx += dxs; ty += dys;
        }
  ...
  _defendTarget(tm, p, b, presser) {
    ...
    /* A descida comeca CEDO, e isso e o essencial. Medido: o alvo do zagueiro
       ja estava certo (9,3 m do gol com a bola a 14 m) e mesmo assim ele
       estava a 19,4 m. Nao e desobediencia — e fisica: o zagueiro corre a no
       maximo ~5 m/s e a bola viaja a ~26 m/s. Quem so comeca a recuar quando
       a bola ja esta perto NUNCA chega. Defesa de verdade nao persegue o
       perigo, ela se antecipa a ele: o bloco afunda desde os 60 m, de modo
       que ao chegar na area ja esteja posicionado em vez de correndo atras. */
    const danger = clamp((60 - D(b.x, b.y, tm.goal.x, tm.goal.y)) / 45, 0, 1);
    if (danger > 0) {
      // âncoras: zaga DENTRO da área (a marca de 8 m fica entre a pequena e a
      // marca do pênalti), meio protegendo a entrada, ataque acompanhando.
      const anchorX = pLine === 'DEF' ? 8 : pLine === 'MID' ? 18 : 30;
      const pullX   = pLine === 'DEF' ? 0.85 : pLine === 'MID' ? 0.55 : 0.22;
      const pullY   = pLine === 'DEF' ? 0.50 : pLine === 'MID' ? 0.32 : 0.12;
  ...
  _integrate(p, tx, ty, dt, freeze) {
    ...
    const hasBallDuty = p._breaking || p._burst || p === this.ball.owner ||
                        (this.ball.traveling && this.ball.receiver === p) ||
                        p._recovering;   // voltar para a área é dever de bola
    let effort;
