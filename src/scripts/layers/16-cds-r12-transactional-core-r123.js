...
P._integrate=function(p,tx,ty,dt,freeze){
  ...
  if(p._gaitPh===undefined)p._gaitPh=sideRand(this,0,6.28);const _bb=this.ball,_dB=_bb?Math.hypot(finite(p.x)-finite(_bb.x),finite(p.y)-finite(_bb.y)):1e9;p.__chase=!p.isGK&&!!_bb&&_dB<14&&(!_bb.owner||_bb.owner.team!==p.team)&&(p===this.teams[p.team]._presser||_dB<2.5);const duty=p._breaking||p._burst||p===this.ball.owner||p.__chase||(this.ball.traveling&&this.ball.receiver===p);let effort;
...
function commitMovement(sim,ctx,corr){if(ctx.committed)return;ctx.committed=true;const dg=state(sim).diagnostics;dg.movementFrames++;for(const tm of sim.teams||[])for(const p of tm.players||[]){if(!p||p.red)continue;const s=ctx.snap.get(p)||{x:finite(p.x),y:finite(p.y)},n=ctx.planned.get(p)||s,c=corr&&corr.get(p)||{x:0,y:0};let x=clamp(n.x+c.x,0,FL),y=clamp(n.y+c.y,0,FW),raw=Math.hypot(x-s.x,y-s.y);dg.maxRawStep=Math.max(dg.maxRawStep,raw);const maxStep=Math.max(.20,finite(p.maxSpd,7)*ctx.dt*1.18+.12);if(raw>maxStep){const q=maxStep/raw;x=s.x+(x-s.x)*q;y=s.y+(y-s.y)*q;dg.movementClamps++;}const fs=Math.hypot(x-s.x,y-s.y);dg.maxFinalStep=Math.max(dg.maxFinalStep,fs);p.x=x;p.y=y;if(ctx.dt>0){p.vx=(x-s.x)/ctx.dt;p.vy=(y-s.y)/ctx.dt;
      ...
      var _vt=finite(p.maxSpd,7)*1.05,_vm=Math.hypot(p.vx,p.vy);
...
P.step=function(dt){const snap=[],beforeHalf=this.half,beforeAdmin=this.dead>0||this.waiting||!!this.pendingRestart;for(const tm of this.teams||[])for(const p of tm.players||[])if(p&&!p.red)snap.push([p,finite(p.x),finite(p.y)]);const r=oldStep.apply(this,arguments),afterAdmin=this.dead>0||this.waiting||!!this.pendingRestart||this.half!==beforeHalf;
  if(!beforeAdmin&&!afterAdmin){const step=clamp(finite(dt,1/30),1/240,.15),dg=state(this).diagnostics;for(const z of snap){const p=z[0],dx=finite(p.x)-z[1],dy=finite(p.y)-z[2],d=Math.hypot(dx,dy),max=Math.max(.30,finite(p.maxSpd,7)*step*1.28+.20);if(d>max){p.x=z[1]+dx/d*max;p.y=z[2]+dy/d*max;p.vx=(p.x-z[1])/step;p.vy=(p.y-z[2])/step;
      /* §R18.40 · mesma correcao na segunda barreira, cuja folga (+0,20) abria
         ate 15,0 m/s. */
      var _gt=finite(p.maxSpd,7)*1.05,_gm=Math.hypot(p.vx,p.vy);
      if(_gm>_gt&&_gm>0){var _gq=_gt/_gm;p.vx*=_gq;p.vy*=_gq;dg.velocityClamps=(dg.velocityClamps||0)+1;}dg.globalMovementClamps++;}dg.maxFinalStep=Math.max(dg.maxFinalStep,Math.hypot(p.x-z[1],p.y-z[2]));}}
...
P._dribble=function(o,d,g){if(!o||!d||!g)return;const tm=this.teams[o.team],stO=ensureStats(this.stats[o.team]),stD=ensureStats(this.stats[d.team]),adv=tm.attackDir>0?o.x:FL-o.x,finalThird=adv>68,wing=o.y<22||o.y>FW-22,dri=attr(o,'drible'),elite=dri>=86||(o.ref&&o.ref.traits&&o.ref.traits.includes('DRIBBLER')),best=typeof this._bestPass==='function'?this._bestPass(o):null,support=best&&best.m&&finite(best.risk)<2.35&&finite(best.score)>.55,attemptP=clamp(.45+(finalThird?.20:0)+(wing?.08:0)+(elite?.17:(dri-65)/320)-(support?.10:0),.32,elite?.88:.76);
  ...
  o._act='dribble';stO.dribblesAttempted++;stD.tackleAttempts++;const S=state(this);S.team[o.team].dribbleAttempts++;S.team[d.team].tackleAttempts++;const ctx=this._actionContext?this._actionContext(o,dist(o,d),'dribble'):{execution:1},dctx=this._actionContext?this._actionContext(d,dist(o,d),'defend'):{execution:1},basePa=facet(o,'drб_atk')*finite(ctx.execution,1)+(wing&&finalThird?4:0)+(elite?4+Math.max(0,dri-86)*.38:0),pd=facet(d,'drб_def')*finite(dctx.execution,1)+2.5,baseRaw=typeof root.duelProb==='function'?root.duelProb(basePa+.5,pd):.5,r1815Carry=this._r1815CarryDuelProfile?this._r1815CarryDuelProfile(o,d,baseRaw,dist(o,d),finalThird):null,pa=basePa+(r1815Carry?r1815Carry.attackDelta:0),raw=typeof root.duelProb==='function'?root.duelProb(pa+.5,pd):baseRaw,p=clamp(raw,.12,elite?.92:.86);if(r1815Carry)this._r1815RecordCarry&&this._r1815RecordCarry(r1815Carry,p);
  if(schance(this,p)){stO.dribblesCompleted++;S.team[o.team].dribblesWon++;const side=schance(this,.5)?1:-1,a=Math.atan2(g.y-o.y,g.x-o.x);o._tx=clamp(o.x+Math.cos(a)*7-Math.sin(a)*3.5*side,1,FL-1);o._ty=clamp(o.y+Math.sin(a)*7+Math.cos(a)*3.5*side,1,FW-1);o.rating+=.12;d.rating-=.05;if(this._r1814MarkRecovery)this._r1814MarkRecovery(d,'dribble',0);this._emit('dribble',{by:o,on:d,ok:true,flair:schance(this,Math.max(.02,Math.min(.46,(dri-72)/72+(elite?.06:0)))),move:(function(_p,_t){try{var _a=(_p&&_p.ref&&_p.ref.a8)||[70,70,70,70,70,70,70,70];var _dri=_a[2],_vel=_a[3],_tec=_a[7],_m=[];if(_vel>=88)_m.push('arrancada');if(_tec>=88)_m.push('drible da vaca');if(_dri>=88&&_tec>=84)_m.push('elástico');if(_dri>=86)_m.push('meia-lua');if(!_m.length)_m.push('corta pra dentro');return _m[(Math.floor((Number(_t)||0)*3)+(Number(_p&&_p.num)||0))%_m.length];}catch(_){return 'corta pra dentro';}})(o,this.t)});return;}
  const ownBox=(this.teams[d.team].attackDir>0?d.x<16.5:d.x>FL-16.5)&&Math.abs(d.y-FW/2)<20;/* §R19.03 · mesmo portao de contato do :16954. Aqui nao existe `timing`, entao o lugar do atraso e ocupado pela diferenca do proprio duelo. */const _r19d = this._r19FoulRisk ? this._r19FoulRisk(d,o,dist(o,d),null,ownBox,clamp((pa-pd+10)/45,0,1)) : null;const foulP = _r19d ? _r19d.p : clamp((this._foulProb?this._foulProb(d):.08)*(ownBox?1.00:1.30),.022,ownBox?.30:.35);d.__r19risco = _r19d ? _r19d.risco : null;if(schance(this,foulP)){this._awardFoul(d,o);return;}const winP=clamp(.52+(pd-pa)/120+(attr(d,'desarme')-70)/170,.30,.82);if(schance(this,winP)){stD.tackles++;stD.tacklesWon++;S.team[d.team].tacklesWon++;if(this._turnover(d)!==false){this._emit('tackle',{by:d,on:o,source:'dribble'});d.rating+=.10;return;}}
...
P._pressAndTackle=function(dt){const o=this.ball&&this.ball.owner;if(!o||!this.teams||!this.stats)return;const defTm=this.teams[1-o.team],near=this._selectPresser?this._selectPresser(defTm,this.ball):null;if(!near)return;const now=finite(this.t),step=clamp(finite(dt,1/30),1/240,.15),nd=dist(o,near),style=defTm.styleKey||'balanced',S=state(this),st=ensureStats(this.stats[near.team]);
  ...
  const radius=clamp(2.42+(attr(near,'aceleracao')-60)/210+(attr(near,'desarme')-65)/230,2.20,2.90);if(nd<=radius&&(!near._tackleCd||near._tackleCd<=0)){const baseRate=style==='press'?8.1:style==='direct'?7.0:style==='counter'?6.2:style==='park'?5.2:style==='tiki'?5.0:6.5,baseDefQ=facet(near,'tackle')*.72+facet(near,'press')*.18+attr(near,'antecipacao')*.10,baseAtkQ=facet(o,'carry')*.78+attr(o,'controle')*.14+attr(o,'compostura')*.08,baseDuelP=typeof root.duelProb==='function'?root.duelProb(baseDefQ,baseAtkQ):clamp(.5+(baseDefQ-baseAtkQ)/140,.18,.82),ownBox=(defTm.attackDir>0?near.x<16.5:near.x>FL-16.5)&&Math.abs(near.y-FW/2)<20,r1815Pressure=this._r1815PressureDuelProfile?this._r1815PressureDuelProfile(near,o,baseDuelP,nd,ownBox):null,defQ=baseDefQ+(r1815Pressure?r1815Pressure.pressureDelta:0),atkQ=baseAtkQ+(r1815Pressure?r1815Pressure.protectionDelta:0),duelP=typeof root.duelProb==='function'?root.duelProb(defQ,atkQ):clamp(.5+(defQ-atkQ)/140,.18,.82),timing=this._r1813TackleTimingProfile?this._r1813TackleTimingProfile(near,o,duelP,clamp((4.9-nd)/4.9,0,1),nd,ownBox):null,rate=baseRate*(timing?timing.multiplier:1),pTry=1-Math.exp(-rate*step);if(timing)this._r1813RecordEvaluation&&this._r1813RecordEvaluation(timing,pTry);if(r1815Pressure)this._r1815RecordPressure&&this._r1815RecordPressure(r1815Pressure,duelP);if(schance(this,pTry)){st.tackleAttempts++;S.team[near.team].tackleAttempts++;near._tackleCd=clamp(.92-(attr(near,'ritmo')-65)/210,.68,1.05);this._emit('tackle_attempt',{by:near,on:o,distance:nd,timingIQ:timing&&timing.timingIQ});const success=clamp(.78+(defQ-atkQ)/205,.65,.93);/* §R19.03 · antes: 0,38 fixo em todo desafio, a 0,5 m ou a 2,5 m, chegando na hora ou atrasado. Agora o contato e portao: sem corpo a corpo nao ha falta. */const _r19 = this._r19FoulRisk ? this._r19FoulRisk(near,o,nd,timing&&timing.timingIQ,ownBox) : null;const foul = _r19 ? _r19.p : clamp((this._foulProb?this._foulProb(near):.08)*(ownBox?.58:1.52),.020,ownBox?.10:.38);near.__r19risco = _r19 ? _r19.risco : null;if(schance(this,foul)){this._awardFoul(near,o);}else if(schance(this,success)){st.tackles++;st.tacklesWon++;S.team[near.team].tacklesWon++;if(nd<=1.98&&this._turnover(near)!==false)this._emit('tackle',{by:near,on:o,source:'press_contact'});else{const dir=defTm.attackDir||1,side=schance(this,.5)?1:-1;this.ball.owner=null;this.ball.lastTouch=near;/* §R19.04 · o clamp em [1,FL-1] impedia o bote de sair. Agora, perto da propria linha e sob pressao, ele bota para fora -- e isso vira escanteio. */if(!(this._r19ClearOut&&this._r19ClearOut(near,'poke'))){if(this._deflectTo)this._deflectTo(clamp(near.x+dir*2.7,1,FL-1),clamp(near.y+side*1.5,1,FW-1),7.2);}this._emit('tackle',{by:near,on:o,source:'press_poke'});}}else{const r1814Dur=this._r1814RecoveryDuration?this._r1814RecoveryDuration(near,.48,'missed_tackle'):.48;near._beatenUntil=now+r1814Dur;const dir=this.teams[o.team].attackDir||1;o._tx=clamp(o.x+dir*1.6,1,FL-1);o._ty=clamp(o.y+(schance(this,.5)?1:-1)*1.2,1,FW-1);o.settle=Math.max(finite(o.settle),.10);this._emit('tackle_missed',{by:near,on:o,distance:nd});}}else if(nd<3.05&&finite(near.__r122ContainAt,-99)+.9<=now){near.__r122ContainAt=now;st.containments++;S.team[near.team].containments++;this._emit('containment',{by:near,on:o,source:'press'});}}
...
try{document.title='Copa dos Sonhos — R15.8';}catch(_){ }
