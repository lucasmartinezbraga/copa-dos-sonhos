...
function threats13(sim,tm){
  ...
  return att.players.filter(a=>{
    ...
    if(L==='FWD')return gd<44||a._runDeep||a._breaking||a===owner;
    return ['CAM','LM','RM','LWB','RWB'].includes(a.slotPos)&&
      (gd<38||a._breaking||a._runDeep||(a===owner&&gd<64));
  }).sort((a,c)=>{
...
P._defendTarget=function(tm,p,b,presser){
  ...
  if(p===tm._cover){
    ...
    let tx=clamp13(b.x+vx/len*7.2,2,FL13-2);const ty=clamp13(b.y+vy/len*7.2,3,FW13-3);
    if(L==='DEF')tx=fromOwnProg13(tm,clamp13(ownProg13(tm,tx),line-3.0,line+3.0));
    return[tx,ty];
  }
  if(p===tm._shadow&&tm._shadowTgt){
    const a=tm._shadowTgt;
    let sx=clamp13(lerp13(b.x,a.x,.40),2,FL13-2);const sy=clamp13(lerp13(b.y,a.y,.40),3,FW13-3);
    if(L==='DEF')sx=fromOwnProg13(tm,clamp13(ownProg13(tm,sx),line-3.0,line+3.0));
    return[sx,sy];
  }
  if(L==='DEF'){
...
try{
  ...
  document.title='Copa dos Sonhos — R15.8';
}catch(_){}
