...
(function (root) {
  ...
  /* Valores promovidos pela grade de calibracao (reports/calib-02.json, 36
     partidas por configuracao, semente base 4200000): 1,89 gol/partida e 37,5%
     de finalizacoes no alvo, contra 1,771 e 36,9% do R19.08. */
  const ERRO_BASE = num(T.erroBase, 14.0);         // dispersao lateral de referencia
  ...
  const DEFESA_BASE = num(T.defesaBase, 0.78);     // chance de defesa com folga zero
    ...
    if (desf.tipo === 'trave' || desf.tipo === 'travessao') {
      ...
      this._startTravel(o, mira, 'shot', () => {
        ...
        if (_chance(calRestart("postCorner", .5))) this._goalKickOrRestart(1 - o.team);
          ...
          const rx = clamp(this.ball.x - dir * _R(3, 9), 2, FL - 2);
          const ry = clamp(this.ball.y + _R(-7, 7), 2, FW - 2);
