...
(function (root) {
  ...
  function body(ctx, o) {
    ...
    // inclinação + PASSADA (vida sem esqueleto): a passada anima as pernas ao
    // deslocar (frequência ∝ distância andada, realista em qualquer velocidade);
    // parado, as pernas descansam na pose original. Corrige o "boneco deslizando".
    let d = dirCache.get(o.key);
    if (!d) { d = { x, y, lean: 0, gait: 0, spd: 0 }; dirCache.set(o.key, d); }
    ...
    const amp = o.divePose ? 0 : clamp(d.spd / 1.5, 0, 1);   // 0 parado … 1 correndo
    const sw = Math.sin(d.gait) * amp;                  // -1..1 alterna as pernas
    const bob = Math.abs(sw) * r * .05;                 // leve sobe-desce do corpo

    ctx.save();
    ctx.translate(x, y - bob);
    if (o.divePose) ctx.rotate(Math.PI / 2);

    // pernas + meias (passada: eleva alternadamente cada perna)
    const llY = r * .40 - Math.max(0, sw) * r * .20;
    const rlY = r * .40 - Math.max(0, -sw) * r * .20;
    ctx.fillStyle = '#17202e';
    ctx.fillRect(-r * .34, llY, r * .26, r * .46);
    ctx.fillRect(r * .08, rlY, r * .26, r * .46);
    ctx.fillStyle = '#0b0f16';
    ctx.fillRect(-r * .36, llY + r * .38, r * .30, r * .22);
    ctx.fillRect(r * .06, rlY + r * .38, r * .30, r * .22);
    // shorts
    ctx.fillStyle = dark;
