...
out=out.replace('<title>Copa dos Sonhos</title>','<title>Copa dos Sonhos — Fase 9 — Build 5.2.0</title>',1)
out=out.replace('Motor 3.0 · futebol simulado lance a lance','Motor 5.2 · inteligência adaptativa do treinador',1)
out='<!-- FASE 9 · INTELIGÊNCIA DO TREINADOR ADVERSÁRIO · ENGINE 5.2.0 -->\n'+out
path=ROOT/'dist/COPA DOS SONHOS - FASE 9 - IA DO TREINADOR - V5.2.0.html'
path.parent.mkdir(parents=True,exist_ok=True);path.write_text(out,encoding='utf-8')
digest=hashlib.sha256(path.read_bytes()).hexdigest()
qa={'phase':9,'engineVersion':'5.2.0','parentEngine':'5.1.0','certifiedBase':'4.3.2','fixedStep':1/60,
'build':str(path.relative_to(ROOT)),'bytes':path.stat().st_size,'sha256':digest,
'features':['manager-profiles','pre-match-plan','live-match-diagnosis','evidence-thresholds','tactical-memory','change-evaluation','automatic-reversal','contextual-substitutions','formation-adaptation','corridor-reinforcement','phase9-manager-ui','manager-data-api'],
