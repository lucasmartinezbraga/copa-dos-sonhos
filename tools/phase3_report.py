...
def main():
    ...
    with (out/'FASE_3_FORMACOES.csv').open('w',newline='',encoding='utf-8-sig') as f:
        w=csv.writer(f);w.writerow(['formacao','jogos','vitorias','empates','delta_gols','delta_xg','delta_chutes','delta_posse'])
    ...
    with (out/'FASE_3_ESTILOS.csv').open('w',newline='',encoding='utf-8-sig') as f:
        keys=['estilo','jogos','winRate','goalDelta','xgDelta','possessionDelta','passCompletionDelta','crossDelta','throughBallDelta','pressWinsDelta','staminaDelta'];w=csv.writer(f);w.writerow(keys)
        for k,v in styles.items():w.writerow([k]+[v.get(x) for x in keys[1:]])
