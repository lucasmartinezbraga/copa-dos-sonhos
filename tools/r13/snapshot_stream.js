...
function loadBuild(path) {
  ...
  scripts.forEach((script, index) => {
    if ([5, 10].includes(index)) return; // carrega contratos 2.5D (idx 4); pula auditor DOM e async-cup
...
// ─── V0: STREAMING DE SNAPSHOTS (read-only) ──────────────────────────────
// Roda uma partida real e coleta VisualState via createVisualStateFromSim a
// cada `stride` passos. Prova que o adapter é somente-leitura: o placar/stats
// permanecem idênticos ao golden e os snapshots vêm congelados (deepFrozen).
const STRIDE = Math.max(1, Number(argv.stride || 1));
const hash = loadBuild(BUILD);
const db = buildDB(DATA);
const C = global.CDS_25D_CONTRACTS;
if (!C || typeof C.createVisualStateFromSim !== 'function')
  throw new Error('CDS_25D_CONTRACTS.createVisualStateFromSim ausente (contratos não carregados)');
...
const frames = [];
let frozenOk = true, hashOk = true, steps = 0;
const FL = Number(global.FL) || 105, FW = Number(global.FW) || 68;
while (!sim.isOver() && steps < 500000) {
  sim.step(1 / 30); steps++;
  if (steps % STRIDE !== 0) continue;
  const vs = C.createVisualStateFromSim(sim, { time: sim.t });
  if (!Object.isFrozen(vs) || !Object.isFrozen(vs.players)) frozenOk = false;
  if (typeof vs.hash !== 'string' || vs.hash.length !== 8) hashOk = false;
  frames.push({
    t: Number(sim.t.toFixed(4)),
    minute: sim.minute | 0,
    ball: {
      x: +vs.ball.position.x.toFixed(3), y: +vs.ball.position.y.toFixed(3), z: +vs.ball.position.z.toFixed(3),
      vx: +vs.ball.velocity.x.toFixed(3), vy: +vs.ball.velocity.y.toFixed(3), vz: +vs.ball.velocity.z.toFixed(3),
      traveling: !!vs.ball.traveling, ownerId: vs.ball.ownerId
    },
    players: vs.players.map(p => ({
      id: p.id, team: p.team,
      x: +p.position.x.toFixed(3), y: +p.position.y.toFixed(3), z: +p.position.z.toFixed(3),
      facing: +p.facing.toFixed(4), pose: p.pose, isGK: p.isGK
    }))
  });
}

// Prova de equivalência read-only: placar/stats vs golden
let goldenMatch = null;
try {
  const gold = JSON.parse(fs.readFileSync(require('path').resolve(__dirname, '../../reports/r13-baseline/golden-diverse-200.json'), 'utf8'));
  const g = gold.matches[index];
  const scoreEq = JSON.stringify(sim.score.slice()) === JSON.stringify(g.score);
  goldenMatch = { index, scoreEq, simScore: sim.score.slice(), goldenScore: g.score, seed: g.seed };
} catch (e) { goldenMatch = { error: String(e.message || e) }; }

const output = {
  meta: {
    version: (global.CDS_R13 && global.CDS_R13.version) || 'unknown',
    sha256: hash, contractsVersion: C.version, seed: Number(row.seed), index,
    formations: [row.homeFormation, row.awayFormation], styles: [row.homeStyle, row.awayStyle],
    field: { FL, FW }, stepDt: 1 / 30, stride: STRIDE, frameCount: frames.length,
    completed: sim.isOver(), score: sim.score.slice()
  },
  proof: { snapshotsFrozen: frozenOk, deterministicHash: hashOk, golden: goldenMatch },
  frames
};
fs.writeFileSync(OUT, JSON.stringify(output));
realConsole.log(JSON.stringify({ out: OUT, frames: frames.length, meta: output.meta, proof: {
  snapshotsFrozen: frozenOk, deterministicHash: hashOk, scoreEqGolden: goldenMatch && goldenMatch.scoreEq
} }, null, 2));
