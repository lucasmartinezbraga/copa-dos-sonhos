...
for(const form of FORMS) for(let i=0;i<N;i++){
  ...
  sim._emit=function(t,d){ if(t==='diag_partidaDef'&&d)A.push(d); return oE.apply(this,arguments); };
  let s=0; while(!sim.isOver()&&s++<500000) sim.step(DT);
}
const st=k=>{const a=A.map(x=>x[k]).filter(v=>Number.isFinite(v)&&v>-9).sort((x,y)=>x-y);
  if(!a.length)return null;const q=p=>+a[Math.min(a.length-1,Math.floor(a.length*p))].toFixed(2);
  return{media:+(a.reduce((x,y)=>x+y,0)/a.length).toFixed(2),p10:q(.10),mediana:q(.50),p90:q(.90)};};
...
realConsole.log(JSON.stringify({build:String(argv.build).split(/[\/]/).pop(), lances:A.length,
  NA_PARTIDA_DO_CRUZAMENTO:{
    voo_m: st('voo'),
    zagueiroMaisProximo_ateOalvo_m: st('defAoAlvo'),
    segundoZagueiro_ateOalvo_m: st('def2AoAlvo'),
    zagueiroMaisProximo_ateOproprioGol_m: st('defAoProprioGol'),
    defensoresDentroDaGrandeArea: avg('defsDentroDaGrandeArea'),
    defensoresAte10mDoAlvo: avg('defsAte10doAlvo'),
    tprDoMaisProximo: st('tprDoMaisProximo'),
    defensoresNaJanela_tpr02a068: avg('tprDentroDaJanela'),
  }},null,2));
