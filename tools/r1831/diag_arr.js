...
for(const form of ['4-3-3','4-4-2','3-5-2']) for(let i=0;i<N;i++){
  ...
  sim._emit=function(t,d){ if(t==='diag_chegada'&&d)A.push(d); return oE.apply(this,arguments); };
  let s=0; while(!sim.isOver()&&s++<400000) sim.step(DT);
}
const avg=k=>A.length?+(A.reduce((s,x)=>s+(x[k]||0),0)/A.length).toFixed(2):0;
const md=k=>{const a=A.map(x=>x[k]).filter(v=>Number.isFinite(v)&&v>=0).sort((x,y)=>x-y);return a.length?+a[Math.floor(a.length/2)].toFixed(1):null;};
const pct=f=>A.length?+(100*A.filter(f).length/A.length).toFixed(1):0;
realConsole.log(JSON.stringify({lances:A.length,
  distDefMaisProximo_mediana:md('distDef'), distGk_mediana:md('distGk'),
  zagDentroDaArea:avg('zagDentroDaArea'), zagAte8mDaBola:avg('zagAte8mDaBola'),
  jogadoresAte6mDaBola:avg('defsAte6'), jogadoresAte12mDaBola:avg('defsAte12'),
  passaValidacao_goleiro_pct:pct(x=>x.gkOk), passaValidacao_defensor_pct:pct(x=>x.defOk),
  ninguemDisputa_pct:pct(x=>!x.gkOk&&!x.defOk)},null,2));
