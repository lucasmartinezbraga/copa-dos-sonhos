...
/* OS-42 · CENSO DE POSSE: quantos passes antes de perder ou finalizar */
const MP=Number(argv.matches||10), DTP=1/30;
let cadeias=0, passes=0, chutes=0, cruzes=0;
const hist={}; const fim={};
let somaDur=0;
for(let i=0;i<MP;i++){
  const sim=novoSim(4200000+i*7919,FORMS[i%FORMS.length]);
  const Pp=Object.getPrototypeOf(sim);
  let cad=null;
  const fecha=(motivo)=>{ if(!cad)return; cadeias++; passes+=cad.p;
    const k=cad.p>=6?'6+':String(cad.p); hist[k]=(hist[k]||0)+1;
    fim[motivo]=(fim[motivo]||0)+1; somaDur+=sim.t-cad.t0; cad=null; };
  sim._emit=function(t,d){
    const r=Pp._emit.apply(this,arguments);
    try{
      const dono=this.ball.owner;
      if(t==='pass'||t==='through'){ if(cad)cad.p++; }
      else if(t==='shot_taken'||t==='header_shot'||t==='low_cross_shot'){chutes++;fecha('finalizacao');}
      else if(t==='cross'){cruzes++;}
      else if(t==='intercept'||t==='tackle'||t==='bad_pass'||t==='miscontrol_out'||t==='header_clear'){fecha('perdeu');}
      else if(t==='throw_in'||t==='goal_kick'||t==='corner'||t==='foul'||t==='offside'||t==='kickoff'){fecha('parou');}
      if(!cad&&dono)cad={team:dono.team,p:0,t0:this.t};
      else if(cad&&dono&&dono.team!==cad.team){fecha('trocou');cad={team:dono.team,p:0,t0:this.t};}
    }catch(_){}
    return r;
  };
...
realConsole.log('  cadeias de posse /partida ',(cadeias/MP).toFixed(1));
realConsole.log('  passes /partida           ',(passes/MP).toFixed(1));
realConsole.log('  passes por cadeia         ',(passes/cadeias).toFixed(2),'  (real ~3 a 4)');
realConsole.log('  duracao media da cadeia   ',(somaDur/cadeias).toFixed(2),'s');
realConsole.log('  chutes /partida           ',(chutes/MP).toFixed(1),'| cruzes',(cruzes/MP).toFixed(1));
realConsole.log('  cadeias por chute         ',(cadeias/chutes).toFixed(1),'  (real ~8 a 10)');
realConsole.log('  passes por cadeia (hist): ',Object.keys(hist).sort().map(k=>k+':'+(hist[k]/cadeias*100).toFixed(0)+'%').join(' '));
realConsole.log('  como a cadeia termina:    ',Object.entries(fim).sort((a,b)=>b[1]-a[1]).map(([k,v])=>k+':'+(v/cadeias*100).toFixed(0)+'%').join(' '));
