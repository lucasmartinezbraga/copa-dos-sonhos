...
/* OS-65e · QUEM ESCREVE p.x. Setter instrumentado num jogador so. */
const H=1/60;
const sim=novoSim(4200000,FORMS[0]);
const alvo=sim.teams[0].players.find(p=>!p.isGK);
let _x=alvo.x, _y=alvo.y;
const conta=new Map();
let n=0;
function reg(){
  n++;
...
Object.defineProperty(alvo,'x',{get(){return _x;},set(v){if(v!==_x)reg();_x=v;},configurable:true});
Object.defineProperty(alvo,'y',{get(){return _y;},set(v){_y=v;},configurable:true});
let st=0;
while(!sim.isOver()&&st++<9000) sim.step(H);
realConsole.log('  escritas em p.x amostradas:',n,'em',st,'quadros');
[...conta.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10).forEach(([k,v])=>{
  realConsole.log('   ',String(v).padStart(7),(v/n*100).toFixed(1)+'%  ',k);
});
